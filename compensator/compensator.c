...
public char passed() {
	...
	// not closed yet
	if (height < _startblk + 403200) {	// test 400 // ) {
		output(1, &result);
		return 0;
	}
...
public void makevote(struct vote * utxo, char * pubkey, char * sig) {
	...
	if (buf[20] != 0 || buf[41] != 0x41)	// not for the net or not pkh script 0x6f for testnet
		fail();
...
char votecheck() {
	...
	if (yes * 100 >= coincount * 95) {
		...
		// not effective yet
		if (height < _startblk + 403200 + 201600)	// test 400 + 200 // )
			return 0;
